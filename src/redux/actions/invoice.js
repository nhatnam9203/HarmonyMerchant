...
export function getBatchHistory(key = "", timeStart = "", timeEnd = "", quickFilter = "", page = 1, isShowLoading = true, isShowLoadMore = false) {
    return {
        ...
        method: 'GET',
        api: `${apiConfigs.BASE_API}settlement/search?key=${key}&timeStart=${timeStart}&timeEnd=${timeEnd}&quickFilter=${quickFilter}&page=${page}&row=10&api-version=1.1`,
        token: true,
        isShowLoading,
        currentPage: page,
        isShowLoadMore
...
export function getCreditBatchDetailById(batchId) {
    return {
        ...
        method: 'GET',
        api: `${apiConfigs.BASE_API}settlement/${batchId}`,
        token: true
    }
}
