      ...
      if (data) {
        ...
        if (action?.isBookingFromCalendar) {
          yield put({
            ...
            appointmentIdBookingFromCalendar: action?.isBookingFromCalendar
              ? data?.mainAppointmentId || 0
              : 0,
          });
        }
        ...
        const subTotal = data.subTotal
          ? formatNumberFromCurrency(data.subTotal)
          : 0;
        ...
        action.isGroup
          ? yield put({
              ...
              api: `appointment/getGroupById/${
                action?.appointmentId || "addItemIntoAppointment"
              }`,
              token: true,
            })
          : yield put({
      ...
      if (!action.isBlock) {
        if (action.isGroup) {
          yield put({
            ...
            method: "GET",
            api: `appointment/getGroupById/${
              action?.appointmentId || "removeItemIntoAppointment"
            }`,
            token: true,
          });
        } else {
  ...
  try {
    ...
    if (parseInt(codeNumber) == 200) {
      yield put({
        ...
        method: "GET",
        api: `appointment/getGroupById/${
          action?.appointmentId || "checkoutAppointment"
        }`,
        token: true,
        appointmentId: action.appointmentId,
        ...
        isNotUpdateCustomerBuyInRedux:
          action?.isNotUpdateCustomerBuyInRedux || false,
      });
    } else if (parseInt(codeNumber) === 401) {
      yield put({
        type: "UNAUTHORIZED",
...
function* paymentAppointment(action) {
  try {
    ...
    if (parseInt(codeNumber) == 200) {
      ...
      if (
        action.paymentMethod !== "harmony" &&
        action.paymentMethod !== "credit_card" &&
        action.paymentMethod !== "debit_card"
      ) {
        yield put({
          type: "CHECKOUT_SUBMIT",
          body: {},
  ...
  try {
    ...
    if (parseInt(codeNumber) == 200) {
      const dueAmount = parseFloat(
        responses?.data?.checkoutPaymentResponse?.dueAmount || 0
      );
      if (dueAmount === 0) {
        yield put({
  ...
  try {
    ...
    if (parseInt(codeNumber) == 200) {
      ...
      yield put({
        ...
        method: "GET",
        api: `appointment/getGroupById/${
          mainAppointmentId ? mainAppointmentId : "removeAppointmentInGroup"
        }`,
        token: true,
      });
    } else if (parseInt(codeNumber) === 401) {
    ...
    if (parseInt(codeNumber) == 200) {
      action.isGroup
        ? yield put({
            ...
            method: "GET",
            api: `appointment/getGroupById/${
              action?.appointmentId || "updateProductInAppointment"
            }`,
            token: true,
          })
        : yield put({
...
function* getCustomerBuyAppointment(action) {
  try {
    ...
    if (parseInt(codeNumber) == 200) {
      ...
      yield put(
        actions.appointment.switchVisibleEnterCustomerPhonePopup(false)
      );
      yield put({
...
function* changeCustomerInAppointment(action) {
  try {
    yield put({ type: "LOADING_ROOT" });
    const state = yield select();
    const { groupAppointment, blockAppointments, customerInfoBuyAppointment } =
      state.appointment;
...
function* handleEnterGiftCardAmount(action) {
  try {
    ...
    if (!addGiftCardInfoAction.bodyAction) {
      ...
      const isNotUpdateCustomerBuyInRedux =
...
export default function* saga() {
  yield all([
    ...
    takeLatest(
      "ADD_GIFT_CARD_INTO_BLOCK_APPOINTMENT",
      addGiftCardIntoBlockAppointment
    ),
