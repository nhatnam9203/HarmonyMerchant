...
class PopupEnterAmountGiftCard extends React.Component {
  ...
  render() {
    const {
      ...
      onRequestClose,
      language,
      visiblePopupGiftCardEnterAmount,
    ...
    const tempTitle =
      addGiftCardInfoAction?.giftCardInfo?.isActive === 1
        ? "Gift Card"
        : "Gift Card Active Amount";
      ...
      >
        ...
        >
          <View style={{ flex: 1 }}>
            ...
            <Button
              onPress={this.addGiftCardAmount}
              style={{
                height: scaleSize(50),
                width: "100%",
  ...
  async componentDidUpdate(prevProps, prevState) {
    ...
    if (
      isUpdateQuantityOfGiftCard &&
      prevProps.isUpdateQuantityOfGiftCard !== isUpdateQuantityOfGiftCard
    ) {
      this.props.actions.appointment.updateQuantityOfGiftCard(false);
      const tempQuantity =
        addGiftCardInfoAction?.giftCardInfo?.isActive === 1
          ? `0.00`
          : `${addGiftCardInfoAction?.giftCardInfo?.amount}`;
...
const mapStateToProps = (state) => ({
  visiblePopupGiftCardEnterAmount:
    state.appointment.visiblePopupGiftCardEnterAmount,
