...
class PopupPaymentDetails extends React.Component {
  ...
  // ---------- Render --------
  render() {
    const { title, visible, onRequestClose, language, paymentDetailInfo } =
      this.props;
    const paidAmounts =
      paymentDetailInfo.paidAmounts && paymentDetailInfo.paidAmounts.length > 0
        ? paymentDetailInfo.paidAmounts[0]
        : {};
...
const ItemDetail = ({ title, value, isBold, subText, style }) => {
  ...
  return (
    <View style={{ height: scaleSize(40), flexDirection: "row" }}>
      <View style={{ flex: 1, justifyContent: "center" }}>
        <Text style={[styles.textCommon, style]}>{title}</Text>
      </View>
