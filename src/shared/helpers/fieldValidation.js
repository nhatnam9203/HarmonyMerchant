...
export default function fieldValidation(state, action) {
  ...
  switch (field) {
     ...
     */
    case 'username': {
      let validUsername = _.isUndefined(
        validate({ username: value }, usernameConstraints),
      );
      if (validUsername) {
        return state
        ...
        return state.setIn(['form', 'fields', 'usernameHasError'], true).setIn(
          ['form', 'fields', 'usernameErrorMsg'],
          'txtValid_user_name',
     ...
     */
    case 'email': {
      let validEmail = _.isUndefined(
        validate({ from: value }, emailConstraints),
      );
      if (validEmail) {
        return state.setIn(['form', 'fields', 'emailHasError'], false);
      } else {
        return state.setIn(['form', 'fields', 'emailHasError'], true).setIn(
          ['form', 'fields', 'emailErrorMsg'],
          'txtValid_email',
     ...
     */
    case 'password': {
      let validPassword = _.isUndefined(
      ...
      if (validPassword) {
        ...
        return state.setIn(['form', 'fields', 'passwordHasError'], true).setIn(
          ['form', 'fields', 'passwordErrorMsg'],
          'txtValid_password',
     ...
     */
    case 'passwordAgain':
      var validPasswordAgain = _.isUndefined(
        validate(
          { password: state.form.fields.password, confirmPassword: value },
