  ...
  return (
    ...
    >
      ...
      <View style={[{ flex: 1, paddingLeft: scaleSize(13) }, styles.container]}>
        <Text style={styles.txt_normal}>{`Name`}</Text>
      </View>
      {/* ---------- Sales -------- */}
      <View style={[{ flex: 0.6 }, styles.container]}>
        <Text style={styles.txt_normal}>{`Sales`}</Text>
      </View>
      {/* ---------- Tax -------- */}
      <View style={[{ flex: 0.5 }, styles.container]}>
        <Text style={styles.txt_normal}>{`Tax`}</Text>
      </View>
      {/* ---------- Tip -------- */}
      <View style={[{ flex: 0.5 }, styles.container]}>
        <Text style={styles.txt_normal}>{`Tip`}</Text>
      </View>
      {/* ---------- Total Sales -------- */}
      <View style={[{ flex: 0.8 }, styles.container]}>
        <Text style={styles.txt_normal}>{`Total Sales`}</Text>
      </View>
    </View>
  );
};
...
export const StaffsItem = ({ staff, onPress }) => {
  return (
    <Button
      onPress={() => onPress(staff?.staffId || 0)}
      style={{
        height: scaleSize(32),
        borderBottomColor: "#DDDDDD",
        borderBottomWidth: 1,
...
export const GiftCardItem = ({ total, onPress }) => {
  return (
    ...
    >
      ...
      <View style={[{ flex: 1, paddingLeft: scaleSize(13) }, styles.container]}>
        <Text
          style={[styles.txt_item, { fontWeight: "500", color: "#0764B0" }]}
        >
          {`Gift Card Sold`}
        </Text>
      </View>
      ...
      >
        <Text
          style={[styles.txt_item, { fontWeight: "600", color: "#0764B0" }]}
        >
          {`$  ${total ? total : 0.0}`}
        </Text>
        <Image
          source={ICON.staff_invoice_blue}
          style={{ marginRight: scaleSize(8) }}
        />
      </View>
    </Button>
    ...
    >
      ...
      <View style={[{ flex: 1 }, styles.container]}>
        <Text
          style={[
            styles.txt_item,
            { fontWeight: "600", color: "#404040", fontSize: scaleSize(14) },
          ]}
        >
          {"Total"}
        </Text>
      ...
      <View
        style={[
          { flex: 0.8, justifyContent: "center", alignItems: "flex-end" },
        ]}
      >
        <Text
          style={[
            styles.txt_item,
            { fontWeight: "600", color: "#4CD964", fontSize: scaleSize(14) },
          ]}
        >
          {`$  ${total ? total : 0.0}`}
        </Text>
  ...
  return (
    ...
    >
      ...
      <View style={{ flex: 1, flexDirection: "row" }}>
        ...
        >
          <Text style={styles.txt_normal}>{`Amount`}</Text>
        </View>
        <View style={{ width: scaleSize(45) }}></View>
      </View>
    </View>
  );
};
      ...
      >
        <View style={{ justifyContent: "center" }}>
          <Text
            style={[
              styles.txt_item,
              { marginLeft: scaleSize(15), color: "#fff", fontWeight: "400" },
              txtStyle,
              titStyle,
            ]}
          >
