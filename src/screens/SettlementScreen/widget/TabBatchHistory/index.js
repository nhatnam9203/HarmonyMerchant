...
class TabBatchHistory extends Layout {
  ...
  gotoCreditPaymentDetail = (settlementDetail) => {
    const paymentTransaction = settlementDetail.paymentTransaction
      ? settlementDetail.paymentTransaction
      : [];
    const paymentByCreditCard = settlementDetail.paymentByCreditCard
      ? settlementDetail.paymentByCreditCard
      : 0.0;
  ...
  didFocus = () => {
    this.batchHistoryListRef.current?.didFocus();
  };
}
