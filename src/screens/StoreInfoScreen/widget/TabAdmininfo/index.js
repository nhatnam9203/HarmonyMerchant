...
class StoreInfoScreen extends Layout {
  constructor(props) {
    ...
    this.state = {
      ...initState,
      businessHour: profile.businessHour
  ...
  addAdmin = () => {
    ...
    for (let i = 0; i < arrayKey.length; i++) {
      if (
        ...
        arrayKey[i] === "lastName" ||
        arrayKey[i] === "displayName" ||
        arrayKey[i] === "pin" ||
        arrayKey[i] === "confirmPin"
      ) {
        if (user[arrayKey[i]] === "") {
          keyError = arrayKey[i];
          break;
    ...
    if (keyError !== "") {
      ...
      const temptStaff = {
        ...
        tipFee: {
          percent: {
            value: parseFloat(
              this.percentTipFeeRef.current?.state.value
                ? this.percentTipFeeRef.current?.state.value
                : 0
            ),
          ...
          fixedAmount: {
            value: parseFloat(
              this.fixedAmountTipFeeRef.current?.state.value
                ? this.fixedAmountTipFeeRef.current?.state.value
                : 0
            ),
        ...
        cashPercent: parseFloat(
          this.cashPercentRef.current?.state.value
            ? this.cashPercentRef.current?.state.value
            : 0
        ),
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (!loading && prevProps.isResetInfoAdmin !== isResetInfoAdmin) {
