...
import { PRINTER_MACHINE } from "@utils";
...
export default class PrintManager {
  ...
  portDiscovery(portType = "Bluetooth") {
    ...
    return Promise.race([
      ...
      new Promise((_, reject) => setTimeout(() => reject("Timeout"), 50000)),
    ]);
  }
    ...
    try {
      ...
      if (PRINTER_MACHINE[portName]) {
        let printResult = await StarPRNT.print(
          PRINTER_MACHINE[portName]?.emulation,
          commands,
          portName
        );
      }
  ...
  async connect(portName, hasBarcodeReader = false) {
    try {
      if (PRINTER_MACHINE[portName]) {
        let connect = await StarPRNT.connect(
          portName,
          PRINTER_MACHINE[portName]?.emulation,
          hasBarcodeReader
        );
