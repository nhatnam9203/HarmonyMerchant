...
class ItemCustomerBasket extends React.Component {
  ...
  showModalDiscount = () => {
    ...
    const checkoutPayments =
      this.props?.paymentDetailInfo?.checkoutPayments || [];
    ...
    if (checkoutPayments.length === 0) {
      ...
      if (appointmentDetail?.giftCards) {
        for (let i = 0; i < appointmentDetail?.giftCards.length; i++) {
          ...
          totalGiftCard =
            parseFloat(totalGiftCard) + parseFloat(l.get(giftCard, "price"));
        }
      }
      this.props.showModalTipAppointment(
        appointmentId,
        tip,
        appointmentDetail?.subTotal - totalGiftCard || 0,
        tipPercent
      );
    }
  };
  ...
  renderHeaderCustomerBaket() {
    ...
    const swipeoutBtns = [
      {
        ...
        component: (
          <Button
            onPress={() =>
              this.props.actions.appointment.removeAppointmentInGroup(
                appointmentId
              )
            }
    ...
    const checkoutPayments = paymentDetailInfo?.checkoutPayments || [];
    const disabledRemoveItemCustomerBasket =
      checkoutPayments.length === 0 ? false : true;
    ...
    return (
      <Swipeout
        ...
        disabled={
          isMain === 1 || isOfflineMode
            ? true
            : disabledRemoveItemCustomerBasket
        }
        close={true}
      >
        <View
          ...
          <View style={{ flexDirection: "row", marginTop: scaleSize(10) }}>
            <View style={{ flex: 1, paddingHorizontal: scaleSize(10) }}>
              ...
              <View style={styles.payNumberTextContainer}>
                <Button
                  style={{ flexDirection: "row" }}
                  onPress={this.showModalTipAppointment.bind(
                    this,
                    temptTip,
                    tipPercent
                  )}
